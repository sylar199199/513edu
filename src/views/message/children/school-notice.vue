<template>
  <div class="school-notice">
    <div v-if="allSystemMsgList.length === 0" class="default-img">
    ...
    <div v-else>
      <ul class="system-list">
        <li
          class="system-item"
          v-for="(item, index, key) in allSystemMsgList"
          :key="key"
          @click="dialog(item.content, item.title, item.createTime)"
        >
          <img
            class="system-img"
            :src="$imgUrl + '/message/list_icon_school_unread%402x.png'"
            alt=""
          />
      ...
      <div class="news-business-page">
        <base-pagenation :total="allSystemMsgTotal" :page-size="5" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState('message', {
      allSystemMsgList: state => state.allSystemMsgList,
      allSystemMsgTotal: state => parseInt(state.allSystemMsgTotal),
    }),
  },
  methods: {
    ...mapActions('message', {
      handleGetDataByCondition: 'getAllSystemMsgList',
    }),
