      ...
      <div class="criticizes-system-main">
        ...
        <div class="criticizes-system-pagination">
          <el-pagination
            ...
            :total="findRecordTotalOneYinJin"
            :page-size="5"
...
export default {
  ...
  computed: {
    ...mapState('myCourse', {
      findRecordListOneYinJin: state => state.findRecordListOneYinJin,
      findRecordTotalOneYinJin: state =>
        parseInt(state.findRecordTotalOneYinJin),
    }),
    newFindRecordList() {
      let newArr = this.findRecordListOneYinJin.map(item => {
        if (item.phone) {
  ...
  methods: {
    ...mapActions('myCourse', ['getFindRecordListOneYinJin']),
    goToList() {
      this.$router.push({
        path: '/unit-testing',
        query: {
    ...
    handleSeach() {
      ...
      this.getFindRecordListOneYinJin(recordReqObj)
    },
    // 分页器
